...
class ImagesWordsRvAdapter : RecyclerView.Adapter<ImagesWordsViewHolder>() {
  ...
  override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ImagesWordsViewHolder {
    val contactView = LayoutInflater.from(parent.context).inflate(R.layout.item_recyclerview_images_words, parent,
        false)
  ...
  override fun onBindViewHolder(holder: ImagesWordsViewHolder, position: Int) {
    ...
    item.imageBase64?.let {
      imagesWordsViewHolder.imageView.setImageBitmap(
          BitmapFactory.decodeByteArray(item.imageBase64, 0, item.imageBase64!!.size))
      ...
      imagesWordsViewHolder.textViewDatetime.text = SimpleDateFormat("yyyy-MM-dd HH:mm:ss z").format(
          Date(item.dateTime))
    }
  }
